...
const Edital = ({edital, setEdital}) => {

    const [searchParams, setSearchParams] = useSearchParams();
    const editalRef = useRef()
    
    useEffect(() => {
        if (edital["pregao"] == searchParams.get("pregao")) {
            editalRef.current.scrollIntoView({behavior: "smooth"})
        }
    }, [])
    ...
    return (
        <div
            ref={editalRef} 
            className={"px-5 pb-5 mb-10 rounded-lg shadow-lg text-sm" +
            (edital["pregao"] == searchParams.get("pregao") ? " bg-indigo-100" : " bg-white")}
        >        
